...
func hashCompactValidatorHelper(validators []CompactValidator, roots [][32]byte, j int, groupSize int, wg *sync.WaitGroup) {
	defer wg.Done()
	for i := range groupSize {
		fRoots, err := CompactValidatorFieldRoots(&validators[j*groupSize+i])
		if err != nil {
...
func OptimizedValidatorRoots(validators []CompactValidator) ([][32]byte, error) {
	...
	wg.Add(n - 1)
	for j := 0; j < n-1; j++ {
		go hashCompactValidatorHelper(validators, roots, j, groupSize, &wg)
	}
	for i := (n - 1) * groupSize; i < len(validators); i++ {
		fRoots, err := CompactValidatorFieldRoots(&validators[i])
		if err != nil {
